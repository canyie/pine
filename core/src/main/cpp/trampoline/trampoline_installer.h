...
namespace pine {
    ...
    public:
        ...
        bool CannotSafeInlineHook(art::ArtMethod *target) {
            ...
            if (UNLIKELY(target_code_size < direct_jump_size)) {
                LOGW("Cannot safe inline hook method: code size of target method too small (size %u)!", target_code_size);
                return true;
            }
    ...
    protected:
        ...
        virtual void *CreateBridgeJumpTrampoline(art::ArtMethod *target, art::ArtMethod *bridge, void *origin_code_entry);
